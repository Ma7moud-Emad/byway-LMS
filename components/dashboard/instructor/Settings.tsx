...
export default function Settings({
  ...
  childOne: ReactNode;
  childTwo: ReactNode;
  childThree: ReactNode;
}) {
  const [active, setActive] = useState<"profile" | "password" | "instructor">(
    "profile",
  );
  ...
  return (
    <>
      <ul className="flex items-center gap-4 font-medium text-lg text-gray-900 bg-white shadow rounded-lg px-4 py-2 mt-4 w-fit capitalize">
        ...
        <li
          className={`px-2 py-1 cursor-pointer rounded-sm ${active == "instructor" && "text-gray-50 bg-blue-500"}`}
          onClick={() => {
